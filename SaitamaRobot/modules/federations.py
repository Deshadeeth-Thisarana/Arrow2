...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @GangOfFriends !"
            )
            return
...
def fed_admin_help(update: Update, context: CallbackContext):
 ...
 • /fedchat`*:* Get all the chats that are connected in the Federation
